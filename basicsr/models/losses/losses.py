...
class PSFloss(nn.Module):
    ...
    def forward(self, inp, gt, kernel, weight=None, **kwargs):
        """
        Args:
            pred (Tensor): of shape (N, C, H, W). Predicted tensor.
            target (Tensor): of shape (N, C, H, W). Ground truth tensor.
        ...
        """
        target = inp
        kernel = kernel.expand(-1, gt.size(1), -1, -1)
        pred = torch.zeros(inp.size()).cuda()
        for i in range(pred.shape[0]):
            pred[i:i+1,:,:,:] = F.conv2d(gt[i:i+1,:,:,:], kernel[i:i+1,:,:,:], padding=(kernel.shape[-2]//2, kernel.shape[-1]//2))
