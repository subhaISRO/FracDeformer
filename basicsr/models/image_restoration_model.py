...
from basicsr.models.archs.frac_fftformer_arch import Discriminator
...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('psf_loss_opt'):
            psf_type = train_opt['psf_loss_opt'].pop('type')
            cri_psf_cls = getattr(loss_module, psf_type)
            self.cri_psf = cri_psf_cls(**train_opt['psf_loss_opt']).to(
                self.device)
    ...
    def setup_optimizers(self):
        ...
        #     if v.requires_grad:
        #         optim_params.append(v)
        # self.optimizer_d = torch.optim.AdamW([{'params': optim_params2}],
        #                                      **train_opt['optim_g'])
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # fakeimages = self.net_g(self.lq)
        # D_loss = 0
        # # for img in fakeimages:
        # D_real = self.net_d(self.gt)
        # D_fake = self.net_d(fakeimages[0])
        # gradient_penalty = self.compute_gradient_penalty(self.net_d, self.gt, fakeimages[0])
        ...
        if self.cri_pix:
            l_pix = 0.
            for pred in preds:
                gt_downsampled2 = F.interpolate(self.gt, scale_factor=0.5)
                gt_downsampled4 = F.interpolate(gt_downsampled2, scale_factor=0.5)
                tot_pix_error = (self.cri_pix(pred[0], self.gt) + self.cri_pix(pred[1], gt_downsampled2) + self.cri_pix(pred[2], gt_downsampled4))/3
                l_pix += tot_pix_error#self.cri_pix(pred, self.gt)
        ...
        if self.cri_fft:
            ...
            for pred in preds:
                gt_downsampled2 = F.interpolate(self.gt, scale_factor=0.5)
                gt_downsampled4 = F.interpolate(gt_downsampled2, scale_factor=0.5)
                tot_fft_error = (self.cri_fft(pred[0], self.gt) + self.cri_fft(pred[1], gt_downsampled2) + self.cri_fft(pred[2], gt_downsampled4))/3
                l_fft += tot_fft_error#self.cri_fft(pred, self.gt)
            l_total += l_fft
        ...
        #     l_psf = 0
        #     for pred in preds:
        #         l_psf = self.cri_psf(self.lq, self.gt, pred[3])  
        #     l_total += l_psf
        #     loss_dict['l_psf'] = l_psf
        
        #WGAN loss
        # During generator forward-backward-update
        # G_loss_wasserstein = 0
        # for pred in preds:
        #     D_fake = self.net_d(pred[0])
        #     G_loss_wasserstein += -torch.mean(D_fake)
        # loss_dict['l_gan'] = G_loss_wasserstein
        
        l_total = l_total #+ 1000 * G_loss_wasserstein
